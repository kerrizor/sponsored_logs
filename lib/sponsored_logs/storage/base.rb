...
module SponsoredLogs
  module Storage
    # Contract for impression storage adapters. Subclass this (or duck-type the
    # three methods) and pass an instance via config.storage to persist
    # impressions wherever you like -- Redis, a database, a file, and so on.
    #
    # The gem computes spend, reports, and the formatted table on top of
    # #snapshot, so an adapter only has to store and return raw tallies.
    #
    class Base
      ...
      # cpm: Float } }. The gem derives everything else from this.
      #
      def snapshot
